class SceneManager {
    constructor(containerId) {
        ...
        this.sculptMode = {
            ...
            falloff: 0.7,
            isSculpting: false,
            sculptedObjects: new Map(),
            lastSculptPoint: null,
            sculptInterval: null,
            highResSegments: 64 // التقسيم العالي للنحت
        };
        
        // إعدادات الأداء للنحت
        this.sculptPerformance = {
            targetFPS: 30,
            frameTime: 1000 / 30,
            lastSculptTime: 0,
    ...
    // ============================================
    // دالات وضع النحت (Sculpting) - المُحسّنة
    // ============================================
    
    setupSculptEvents() {
        const canvas = this.renderer.domElement;
        
        canvas.addEventListener('mousedown', (e) => this.onSculptStart(e));
        canvas.addEventListener('mousemove', (e) => this.onSculptMove(e));
        canvas.addEventListener('mouseup', () => this.onSculptEnd());
        canvas.addEventListener('mouseleave', () => this.onSculptEnd());
        
        if (this.isTouchDevice) {
            canvas.addEventListener('touchstart', (e) => {
                if (this.sculptMode.enabled && e.touches.length === 1) {
                    e.preventDefault();
            ...
            canvas.addEventListener('touchend', () => this.onSculptEnd());
        }
    }
    
    onSculptStart(event) {
        if (!this.sculptMode.enabled) return;
        if (!this.selectedObject || !this.isSculptable(this.selectedObject)) {
            this.showNotification('يجب تحديد مكعب أولاً للنحت', 'warning');
            return;
        }
        
        // ✅ التحقق إذا كان المكعب بحاجة لتقسيم عالي (أول نحت)
        if (!this.selectedObject.userData.isHighRes && !this.selectedObject.userData.isSubdivided) {
            this.subdivideForSculpting();
            // ننتظر انتهاء التقسيم قبل النحت
            return;
        }
        
        this.sculptMode.isSculpting = true;
        ...
        this.performSculpt(event);
        this.sculptPerformance.lastSculptTime = performance.now();
    }
    
    onSculptMove(event) {
        if (!this.sculptMode.enabled || !this.sculptMode.isSculpting) return;
        
        const now = performance.now();
        const elapsed = now - this.sculptPerformance.lastSculptTime;
        
        if (elapsed >= this.sculptPerformance.frameTime) {
            this.performSculpt(event);
            this.sculptPerformance.lastSculptTime = now;
        }
    }
    ...
    onSculptEnd() {
        ...
        if (this.sculptMode.isSculpting) {
            this.sculptMode.isSculpting = false;
            ...
            if (this.selectedObject) {
                this.selectedObject.geometry.computeVertexNormals();
                this.selectedObject.geometry.attributes.position.needsUpdate = true;
                this.saveHistoryState();
            }
        }
    }
    
    // ✅ دالة جديدة: تقسيم المكعب تلقائياً للنحت
    subdivideForSculpting() {
        if (!this.selectedObject) return;
        
        this.showLoading('جاري تحضير المكعب للنحت...');
        
        // استخدام setTimeout للسماح بإظهار رسالة التحميل
        setTimeout(() => {
            try {
                const geometry = this.selectedObject.geometry;
                const oldMaterial = this.selectedObject.material;
                const oldMatrix = this.selectedObject.matrixWorld.clone();
                
                // ✅ التقسيم العالي: 64×64×64 للنعومة المثالية
                const segments = this.sculptMode.highResSegments;
                ...
                // إنشاء كائن جديد بنفس الخصائص
                const newObject = new THREE.Mesh(newGeometry, newMaterial);
                newObject.position.copy(this.selectedObject.position);
                newObject.rotation.copy(this.selectedObject.rotation);
                newObject.scale.copy(this.selectedObject.scale);
                newObject.name = this.selectedObject.name;
                newObject.userData = { 
                    ...this.selectedObject.userData,
                    isHighRes: true, // ✅ علامة أنه مكعب عالي الدقة
                    originalSegments: 16 // حفظ القيمة الأصلية
                };
                newObject.castShadow = true;
                newObject.receiveShadow = true;
                
                // استبدال الكائن القديم
                ...
                this.hideLoading();
                this.updateSculptStats();
                this.showNotification(
                    `✅ تم تحضير المكعب للنحت: ${segments}×${segments}×${segments} جزء`,
                    'success'
                );
                ...
                this.hideLoading();
                console.error('خطأ في تقسيم المكعب:', error);
                this.showNotification('خطأ في تحضير المكعب للنحت', 'error');
            }
        }, 50);
    }
    ...
    performSculpt(event) {
        ...
        const intersects = this.raycaster.intersectObject(this.selectedObject, false);
        ...
        if (intersects.length > 0) {
            ...
            const blendFactor = Math.abs(worldNormal.y);
            const sculptDirection = new THREE.Vector3().lerpVectors(worldNormal, upVector, 0.7).normalize();
            
            const brushCenter = point;
            const brushRadius = this.sculptMode.brushSize;
            const intensity = this.sculptMode.intensity;
            const falloff = this.sculptMode.falloff;
            const tool = this.sculptMode.tool;
            
            const worldMatrix = this.selectedObject.matrixWorld;
            const inverseMatrix = new THREE.Matrix4().copy(worldMatrix).invert();
            const localBrushCenter = brushCenter.clone().applyMatrix4(inverseMatrix);
            const localSculptDir = sculptDirection.clone().transformDirection(inverseMatrix).normalize();
            
            const tempVertex = new THREE.Vector3();
            const tempWorldVertex = new THREE.Vector3();
            
            // Gaussian falloff للنعومة
            const applySculpt = (distance) => {
                if (distance >= brushRadius) return 0;
                const t = distance / brushRadius;
                const influence = Math.exp(-(t * t) * 3);
                if (falloff > 0) {
                    return influence * Math.pow(1 - t, falloff);
                }
                return influence;
            };
            ...
            for (let i = 0; i < positions.count; i++) {
                ...
                tempWorldVertex.copy(tempVertex).applyMatrix4(worldMatrix);
                
                const distance = tempWorldVertex.distanceTo(brushCenter);
                
                if (distance < brushRadius) {
                    const influence = applySculpt(distance);
                    
                    if (influence > 0.001) {
                        const displacement = influence * intensity * 0.15;
                        
                        if (tool === 'raise') {
                            tempVertex.addScaledVector(localSculptDir, displacement);
                        } else {
                            tempVertex.addScaledVector(localSculptDir, -displacement);
                        }
            ...
            if (!this.sculptMode.sculptInterval) {
                this.sculptMode.sculptInterval = setTimeout(() => {
                    geometry.computeBoundingSphere();
                    geometry.computeBoundingBox();
                    this.sculptMode.sculptInterval = null;
                }, 100);
            }
        }
    }
    ...
    isSculptable(object) {
        ...
        const isBox = object.geometry.type === 'BoxGeometry' || 
                      object.geometry.type === 'BoxBufferGeometry';
        
        const isNative = object.userData?.isNativeObject === true;
        
        return isBox && isNative;
    }
    
    enableSculptMode(enabled) {
        this.sculptMode.enabled = enabled;
        
        const container = document.getElementById('scene-container');
        const indicator = document.getElementById('sculpt-indicator');
        const sculptBtn = document.getElementById('toggle-sculpt');
        const sculptOptions = document.getElementById('sculpt-options');
        const toggle = document.getElementById('sculpt-mode-toggle');
        
        if (enabled) {
            container.classList.add('sculpt-mode');
            indicator.classList.remove('hidden');
            sculptBtn.classList.add('active');
            sculptOptions.classList.remove('hidden');
            ...
            if (this.selectedObject && !this.isSculptable(this.selectedObject)) {
                this.deselectObject();
                this.showNotification('اختر مكعباً للنحت عليه', 'info');
            } else if (this.selectedObject && !this.selectedObject.userData.isHighRes) {
                // ✅ إظهار تنبيه أن المكعب سيتم تقسيمه عند أول نحت
                this.showNotification('💡 المكعب سيتم تقسيمه تلقائياً عند أول نحت', 'info');
            }
            ...
            this.updateSculptUI();
            this.showNotification('تم تفعيل وضع النحت - الكاميرا متوقفة', 'sculpt');
            
        } else {
            container.classList.remove('sculpt-mode');
            indicator.classList.add('hidden');
            sculptBtn.classList.remove('active');
            sculptOptions.classList.add('hidden');
    ...
    setSculptTool(tool) {
        this.sculptMode.tool = tool;
        this.updateSculptUI();
    }
    
    setSculptIntensity(value) {
        this.sculptMode.intensity = parseFloat(value);
        document.getElementById('sculpt-intensity-value').textContent = value;
    }
    
    setSculptBrushSize(value) {
        this.sculptMode.brushSize = parseFloat(value);
        document.getElementById('sculpt-brush-size-value').textContent = value;
    }
    
    setSculptFalloff(value) {
        this.sculptMode.falloff = parseFloat(value);
        document.getElementById('sculpt-falloff-value').textContent = value;
    }
    
    updateSculptUI() {
        const toolText = this.sculptMode.tool === 'raise' ? 'رفع' : 'حفر';
        const toolIcon = this.sculptMode.tool === 'raise' ? '⬆️' : '⬇️';
        
        const modeText = document.getElementById('sculpt-mode-text');
        const toolIconEl = document.getElementById('sculpt-tool-icon');
        
        if (modeText) modeText.textContent = `وضع النحت: ${toolText}`;
        if (toolIconEl) toolIconEl.textContent = toolIcon;
        
        const raiseBtn = document.getElementById('sculpt-raise');
        const lowerBtn = document.getElementById('sculpt-lower');
        
        if (raiseBtn && lowerBtn) {
            raiseBtn.classList.toggle('active', this.sculptMode.tool === 'raise');
            lowerBtn.classList.toggle('active', this.sculptMode.tool === 'lower');
        }
    }
    ...
    // ✅ زر التقسيم اليدوي (اختياري)
    subdivideSelectedMesh() {
        if (!this.selectedObject || !this.isSculptable(this.selectedObject)) {
            this.showNotification('يجب تحديد مكعب أولاً', 'warning');
            return;
        }
        
        if (this.selectedObject.userData.isHighRes) {
            this.showNotification('المكعب مقسم بالفعل!', 'info');
            return;
        }
    ...
    saveObjectStateForUndo(object) {
        if (!object || !object.geometry) return;
        
        const positions = object.geometry.attributes.position.array.slice();
        this.sculptMode.sculptedObjects.set(object.uuid, {
            positions: positions,
            timestamp: Date.now()
        });
    }
    ...
    // ============================================
    // الدالات الأصلية (الموجودة سابقاً)
    // ============================================
    ...
    selectObject(object) {
        ...
        const isSculptable = this.isSculptable(object);
        const isHighRes = object.userData?.isHighRes;
        ...
        if (object.material) {
            if (Array.isArray(object.material)) {
                ...
                object.material.forEach(mat => {
                    mat.emissive = new THREE.Color(isSculptable ? 0x440044 : 0x222222);
                });
            } else {
                object.userData.originalEmissive = object.material.emissive ? 
                    object.material.emissive.clone() : new THREE.Color(0x000000);
                object.material.emissive = new THREE.Color(isSculptable ? 0x440044 : 0x222222);
            }
        }
        
        if (this.sculptMode.enabled) {
            if (!isSculptable) {
                this.showNotification('هذا الكائن غير قابل للنحت. استخدم المكعبات فقط.', 'warning');
                this.transformControls.visible = false;
            } else {
                this.transformControls.visible = false;
                this.updateSculptStats();
                ...
                if (isHighRes) {
                    this.showNotification('✨ المكعب جاهز للنحت الناعم!', 'sculpt');
                } else {
                    this.showNotification('💡 اضغط للنحت - سيتم التقسيم تلقائياً', 'info');
                }
            }
        } else {
            if (this.transformControls) {
    ...
    setTransformMode(mode) {
        ...
        if (this.transformControls && this.selectedObject && !this.sculptMode.enabled) {
            switch(mode) {
                case 'translate':
    ...
    showTransformControls() {
        ...
        if (sceneControls && !this.sculptMode.enabled) {
    ...
    updateObjectInfo() {
        ...
        const isSculptable = this.isSculptable(this.selectedObject);
        const isHighRes = this.selectedObject.userData?.isHighRes;
        let sculptBadge = '';
        
        if (isSculptable) {
            if (isHighRes) {
                sculptBadge = ' <span style="color: #9b59b6;">(مجهز للنحت ✨)</span>';
            } else {
                sculptBadge = ' <span style="color: #3498db;">(قابل للنحت 🎨)</span>';
            }
        }
        ...
        if (infoDiv) {
            ...
            if (this.selectedObject.userData?.isImported) {
                ...
                typeText = 'كائن أصلي' + (isSculptable ? (isHighRes ? ' - مجهز للنحت' : ' - قابل للنحت') : '');
            }
            
            infoDiv.innerHTML = `
                <small>
                    <strong>${this.selectedObject.name || 'كائن'}${sculptBadge}</strong><br>
                    النوع: ${typeText}<br>
        ...
        if (nameDiv) {
            let statusIcon = '';
            if (isSculptable) {
                statusIcon = isHighRes ? ' ✨' : ' 🎨';
            }
            nameDiv.textContent = `الكائن المحدد: ${this.selectedObject.name || 'غير مسمى'}${statusIcon}`;
        }
    }
    ...
    addObject(shapeType) {
        ...
        switch(shapeType) {
            case 'cube':
                // ✅ مكعب عادي بتقسيم قليل للأداء (16×16×16)
        ...
        if (this.sculptMode.enabled && shapeType === 'cube') {
            this.updateSculptStats();
        }
    ...
    deleteSelectedObject() {
        if (this.selectedObject) {
            ...
            this.sculptMode.sculptedObjects.delete(this.selectedObject.uuid);
    ...
    duplicateSelectedObject() {
        if (this.selectedObject) {
            ...
            if (this.selectedObject.userData) {
                ...
                // ✅ إعادة تعيين حالة النحت للنسخة
                clone.userData.isHighRes = false;
            }
            
            if (this.selectedObject.geometry) {
                clone.geometry = this.selectedObject.geometry.clone();
            }
    ...
    serializeObject(obj) {
        return {
            ...
            userData: {
                isNativeObject: true,
                isHighRes: obj.userData?.isHighRes || false
            }
        };
    }
    ...
    deserializeObject(objData) {
        ...
        // ✅ استعادة التقسيم المناسب حسب الحالة المحفوظة
        const segments = objData.userData?.isHighRes ? 64 : 16;
    ...
    clearSceneSilently() {
        ...
        this.sculptMode.sculptedObjects.clear();
