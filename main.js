...
function initEventHandlers() {
    try {
        ...
        // زر وضع النحت
        const toggleSculptBtn = document.getElementById('toggle-sculpt');
        if (toggleSculptBtn) {
            toggleSculptBtn.addEventListener('click', toggleSculptMode);
        }
...
// إعداد أحداث وضع النحت
function setupSculptControls() {
    if (!sceneManager) return;
    
    // تفعيل/إلغاء وضع النحت
    const sculptToggle = document.getElementById('sculpt-mode-toggle');
    if (sculptToggle) {
        sculptToggle.addEventListener('change', (e) => {
            sceneManager.enableSculptMode(e.target.checked);
        });
    }
    ...
    if (raiseBtn) {
        raiseBtn.addEventListener('click', () => {
            sceneManager.setSculptTool('raise');
        });
    }
    
    if (lowerBtn) {
        lowerBtn.addEventListener('click', () => {
            sceneManager.setSculptTool('lower');
        });
    }
    
    // سلايدرات النحت
    setupSliderControl('sculpt-intensity', 'sculpt-intensity-value', (value) => {
        sceneManager.setSculptIntensity(value);
    });
    
    setupSliderControl('sculpt-brush-size', 'sculpt-brush-size-value', (value) => {
        sceneManager.setSculptBrushSize(value);
    });
    
    setupSliderControl('sculpt-falloff', 'sculpt-falloff-value', (value) => {
        sceneManager.setSculptFalloff(value);
    });
    
    // زر تقسيم المكعب
    const subdivideBtn = document.getElementById('subdivide-mesh');
    if (subdivideBtn) {
        subdivideBtn.addEventListener('click', () => {
            sceneManager.subdivideSelectedMesh();
        });
    }
}
...
function toggleSculptMode() {
    ...
    const newState = !sceneManager.sculptMode.enabled;
    sceneManager.enableSculptMode(newState);
}
...
function updateSculptStats() {
    if (!sceneManager) return;
    
    // تحديث إحصائيات النحت إذا كان الكائن المحدد قابلاً للنحت
    if (sceneManager.selectedObject && sceneManager.isSculptable(sceneManager.selectedObject)) {
        sceneManager.updateSculptStats();
    }
}
